...
loadMyStats()
{
  filename = "br_stats/" + self.guid + ".dat";
  if(fexists(filename)) {
    file = fopen(filename);
    if(file != -1) {
      data = fread(file);
      ...
      wait 1;
      self iprintln("Loaded! Points: ^7" + self.stats["points"] + " Total Kills: ^7" + self.stats["totalKills"] + " Total Deaths: ^7" + self.stats["totalDeaths"]); //Will remove it later
    }
    else {
    ...
    level iprintln("this stats file doesn't exist... yet. ^7" + filename);
  }
}
...
saveMyStats()
{
  ...
  if(fexists(filename)) {
    ...
    if(file != -1) {
      ...
      level iprintln("error opening ^7" + filename);
      return;
    }
    fclose(file);
  }
